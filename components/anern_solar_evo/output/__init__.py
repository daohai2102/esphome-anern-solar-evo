...
AnernSolarEvoOutput = anern_solar_evo_ns.class_("AnernSolarEvoOutput", output.FloatOutput)
SetOutputAction = anern_solar_evo_ns.class_("SetOutputAction", automation.Action)
...
CONFIG_SCHEMA = ANERN_SOLAR_EVO_COMPONENT_SCHEMA.extend(
    {
        cv.Optional(type): output.FLOAT_OUTPUT_SCHEMA.extend(
            {
                cv.Required(CONF_ID): cv.declare_id(AnernSolarEvoOutput),
...
@automation.register_action(
    "output.anern_solar_evo.set_level",
    SetOutputAction,
    cv.Schema(
        {
...
def output_anern_solar_set_level_to_code_evo(config, action_id, template_arg, args):
